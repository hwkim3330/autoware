...
DKD "export DISPLAY=:1 XAUTHORITY=/root/.Xauthority; source /opt/autoware/setup.bash; rviz2 -d /opt/autoware/share/autoware_launch/rviz/autoware.rviz --ros-args -p use_sim_time:=true > /tmp/rviz.log 2>&1"
...
echo "Soongsil replay up: bag=$BAG | app=com.keti.soongsil | rviz=autoware.rviz(standard,map) on :1"
