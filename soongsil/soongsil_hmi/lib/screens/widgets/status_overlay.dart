...
class StatusOverlay extends StatelessWidget {
  ...
  Widget _sensorRow() {
    ...
    final sensors = s.sensors.isEmpty
        ...
        : s.sensors;
    // ROii LiDAR suite: live per-sensor health from the gateway when the
    // 4-lidar sim is running (front/rear G32, side rotating Pandars).
    const lidarShort = {
      'FrontCenterLidar': 'G32-F', 'RearCenterLidar': 'G32-R',
      'FrontLeftLidar': 'PANDAR-L', 'FrontRightLidar': 'PANDAR-R',
    };
    return Row(children: [
      for (final e in sensors.entries) chip(e.key.toUpperCase(), e.value),
      if (s.parts.length > 1)
        for (final e in s.parts.entries)
          chip(lidarShort[e.key] ?? e.key, e.value),
    ]);
  }
