...
class InitializeInterface(object):
    ...
    def load_world(self):
        ...
        # NIRO real-map support: if carla_map is a .xodr path (OSM-derived
        # OpenDRIVE for 숭실대/판교/etc.), generate the world from it instead of
        # loading a prebuilt Town. The OSM pipeline (scripts/osm_to_carla.py)
        # produces these. Otherwise behave exactly as stock (load a Town).
        if self.carla_map.endswith(".xodr") and os.path.exists(self.carla_map):
            with open(self.carla_map, "r") as _f:
                _xodr = _f.read()
            _params = carla.OpendriveGenerationParameters(
                vertex_distance=2.0, max_road_length=200.0, wall_height=0.0,
                additional_width=0.6, smooth_junctions=True,
                enable_mesh_visibility=True)
            client.generate_opendrive_world(_xodr, _params)
        else:
